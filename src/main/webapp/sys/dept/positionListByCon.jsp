...
<div class="rightinfo">
    ...
    <table class="tablelist">
        ...
        <tbody>
        <c:forEach items="${listJobByCon}" var="ljbc">
            <tr>
                <td>${ljbc.jobId}</td>
                <td>${ljbc.jobName}</td>
                <td>${ljbc.dept.deptName}</td>
                <td>${ljbc.jobState}</td>
                <td>
                    <a href="${pageContext.request.contextPath}/getOneJob.do?jid=${ljbc.jobId}" class="tablelink">修改</a>
                        <%--<a href="positionGrant.jsp" class="tablelink">赋权</a>--%>
                    <c:if test="${ljbc.jobState=='正常'}">
                        <a href="javascript:void(0)" class="tablelink" onclick="tipOpen('是否确认注销这个职位？',${ljbc.jobId})">注销</a>
                    </c:if>
                    <c:if test="${ljbc.jobState=='注销'}">
                        <a href="javascript:void(0)" class="tablelink" onclick="tipOpen01('是否确认恢复这个职位？',${ljbc.jobId})">恢复</a>
                    </c:if>
                </td>
            </tr>
        </c:forEach>
    ...
    <div class="pagin">
        <div class="message">共<i class="blue">${countJobByCon}</i>条记录，当前显示第&nbsp;<i class="blue">${pageIndex}&nbsp;</i>页</div>
        <ul class="paginList">
            <li class="paginItem"><a href="${pageContext.request.contextPath }/getJobByCon.do?pageIndex=1&jobName=${jobName}&deptId=${deptId}">首页</a></li>
            <li class="paginItem"><a href="${pageContext.request.contextPath }/getJobByCon.do?pageIndex=${pageIndex-1}&jobName=${jobName}&deptId=${deptId}">上页</a></li>
            <li class="paginItem"><a href="${pageContext.request.contextPath }/getJobByCon.do?pageIndex=${pageIndex+1}&jobName=${jobName}&deptId=${deptId}">下页</a></li>
            <li class="paginItem"><a href="${pageContext.request.contextPath }/getJobByCon.do?pageIndex=${rowJobByCon}&jobName=${jobName}&deptId=${deptId}">末页</a></li>
        </ul>
    </div>
    <!-- 提示框 -->
